...
Hello world example for Kserve
...
## Setting up kserve with huggingface model

In the terminal of your notebook namespace run:
```bash
kubectl apply -f kserve-service.yaml
```
where `kserve-service.yaml` with addional comments to the their configuration
...
metadata:
  annotations:
    sidecar.istio.io/inject: "false" # Disables injection of an Istio sidecar proxy. Istio is often used for traffic management, but here it is turned off
